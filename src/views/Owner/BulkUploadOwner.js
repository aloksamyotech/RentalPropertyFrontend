...
const BulkUploadOwner = ({ open, onClose, data }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        await postApi(urls.owner.bulkUploadOwner, formData, customHeaders);
        
        formik.resetForm();
        onClose();
        toast.success(t('owner uploaded successfully!'));
      } catch (error) {
        console.error('Upload error:', error);
        toast.error(error.response?.data?.message || t('Failed to upload owner!'));
      }
    }
  });
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      >
        <Typography variant="h6">{t('Bulk Upload owner')}</Typography>
...
export default BulkUploadOwner;
